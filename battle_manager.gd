...
func _on_spell_cast_initiated(spell_card: Node2D):
	...
	if spell_name == "Início da Peste":
	...
	elif spell_name == "Escritos Populares":
		var restrictions = {"type": "Criatura"}
		setup_targeting_state(spell_card, 0, 1, restrictions, true)
	elif spell_name == "Desordem":
		var opponent_bm = get_opponent_battle_manager()
		var opponent_peer_id = get_opponent_peer_id()
		if is_instance_valid(opponent_bm) and opponent_peer_id != 0:
			opponent_bm.rpc_id(opponent_peer_id, "rpc_opponent_cast_global_spell", spell_name)
	...
	elif spell_name == "Legado Incompreendido":
		var has_ally_creature = false
		for card in player_cards_on_battlefield:
			if is_instance_valid(card) and card.card_type == "Criatura":
				has_ally_creature = true
				break
		if not has_ally_creature:
			if has_method("spawn_floating_text"):
				spawn_floating_text("Precisa de Sacrifício!", spell_card.global_position, Color.RED)
			player_current_energy += spell_card.energy_cost
			update_energy_labels()
			if is_instance_valid(spell_card) and is_instance_valid(spell_card.player_hand_ref):
				spell_card.visible = true
				spell_card.player_hand_ref.call_deferred("add_card_to_hand", spell_card)
			else:
				spell_card.queue_free()
			reset_targeting_state()
			return
		var restrictions = {"type": "Criatura", "side": "ally"}
		setup_targeting_state(spell_card, 1, 1, restrictions, true)
	elif spell_name == "Gastheorie":
		var opponent_bm = get_opponent_battle_manager()
		var opponent_peer_id = get_opponent_peer_id()
		if is_instance_valid(opponent_bm) and opponent_peer_id != 0:
			opponent_bm.rpc_id(opponent_peer_id, "rpc_opponent_cast_global_spell", spell_name)
		if spell_card.ability_script != null:
			...
			print("ERRO: 'Vorlesungen' sem ability_script carregado!")
		reset_targeting_state()
...
func start_turn(player_or_opponent: String):
	...
	for card in player_cards_on_battlefield:
		if is_instance_valid(card) and card.has_method("remove_temp_buffs"):
			card.remove_temp_buffs()
	...
	if player_or_opponent == "Jogador":
		...
		for card in player_cards_on_battlefield:
			if is_instance_valid(card) and is_instance_valid(card.ability_script):
				if card.ability_script.has_method("trigger_turn_start"):
					await card.ability_script.trigger_turn_start(self, card)
...
func confirm_attackers():
	...
	for i in range(declared_attackers.size() - 1, -1, -1):
		var attacker = declared_attackers[i]
		if is_instance_valid(attacker.ability_script) and attacker.ability_script.has_method("should_cancel_combat_action"):
			if attacker.ability_script.should_cancel_combat_action(self, attacker):
				if attacker.has_method("show_attack_indicator"):
					attacker.show_attack_indicator(false)
				declared_attackers.remove_at(i)
...
func confirm_blockers():
	...
	var blockers_to_remove = []
	for blocker in blocker_assignments.keys():
		if is_instance_valid(blocker.ability_script) and blocker.ability_script.has_method("should_cancel_combat_action"):
			if blocker.ability_script.should_cancel_combat_action(self, blocker):
				blockers_to_remove.append(blocker)
	for blocker in blockers_to_remove:
		var attacker = blocker_assignments[blocker]
		blocker_assignments.erase(blocker)
		if declared_blockers.has(attacker):
			declared_blockers[attacker].erase(blocker)
			if declared_blockers[attacker].is_empty():
				declared_blockers.erase(attacker)
		if is_instance_valid(blocker):
			blocker.show_block_indicator(false)
...
func handle_spell_target_selection(target_card: Node2D):
	...
	if valid_target and current_spell_target_restrictions.has("side"):
		var is_my_card = player_cards_on_battlefield.has(target_card)
		if current_spell_target_restrictions.side == "ally" and not is_my_card:
			valid_target = false
			print("Alvo inválido: Deve ser uma carta aliada.")
		elif current_spell_target_restrictions.side == "enemy" and is_my_card:
			valid_target = false
			#spawn_floating_text("Alvo deve ser uma carta inimiga", )
			print("Alvo inválido: Deve ser uma carta inimiga.")
...
func spawn_floating_text(text: String, position: Vector2, color: Color = Color.WHITE):
	if FLOATING_TEXT_SCENE:
		var float_txt = FLOATING_TEXT_SCENE.instantiate()
		float_txt.global_position = position
		if is_instance_valid(card_manager):
			card_manager.add_child(float_txt)
		else:
			get_tree().root.add_child(float_txt)
		float_txt.set_text(text, color)
